...
export const DisplayRoomConflicts: FC<{
  ...
  const conflictsByRoom = roomConflicts.reduce(
    (acc, conflict) => {
      const roomKey = `${conflict.conflictingMeetInfo.building} ${conflict.conflictingMeetInfo.room}`
      if (!acc[roomKey]) {
        acc[roomKey] = []
      }
      acc[roomKey].push(conflict)
      return acc
    },
    {} as Record<string, ConflictInfo[]>,
  )

  const uniqueRooms = Object.keys(conflictsByRoom)
          ...
          {uniqueRooms.map((room, roomIdx) => (
            <div key={roomIdx}>
              ...
              <ul className="space-y-1 text-rose-200/80 ml-2">
                {conflictsByRoom[room].map((conflict, conflictIdx) => (
                  <li key={conflictIdx}>
